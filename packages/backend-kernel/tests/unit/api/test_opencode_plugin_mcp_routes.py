...
from app.api import opencode_plugin_mcp_routes as routes
from app.services.plugin_backend_mcp_proxy_service import PluginBackendMcpProxyError


class _ProxyServiceStub:
    def __init__(self, payload=None, error: PluginBackendMcpProxyError | None = None) -> None:
...
async def test_initialize_returns_local_server_info() -> None:
    ...
    response = await routes.opencode_plugin_backend_mcp("com.demo.plugin", body)

    assert response["result"]["serverInfo"]["name"] == "dawnchat-opencode-plugin-backend-mcp-proxy"
...
async def test_tools_list_forwards_to_proxy(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(routes, "get_plugin_backend_mcp_proxy_service", lambda: _ProxyServiceStub(payload=expected))
...
async def test_tools_call_returns_error_when_proxy_fails(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr(
        ...
        lambda: _ProxyServiceStub(
            error=PluginBackendMcpProxyError(code="plugin_backend_unavailable", message="missing endpoint")
        ),
    )
    body = routes.JsonRpcRequest(id=2, method="tools/call", params={"name": "demo.echo", "arguments": {}})
