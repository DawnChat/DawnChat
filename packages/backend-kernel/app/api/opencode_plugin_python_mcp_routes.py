...
logger = get_logger("opencode_plugin_python_mcp_routes")
    ...
    if body.method == "initialize":
        response = jsonrpc_response(
            body.id,
            {
                ...
                "capabilities": {"tools": {}},
                "serverInfo": {
                    "name": "dawnchat-opencode-plugin-python-mcp-proxy",
                    "version": "0.1.0",
                },
            },
        )
        ...
        try:
            response = await get_plugin_python_mcp_proxy_service().forward_jsonrpc(
                plugin_id=plugin_id,
                payload=payload,
            )
        except PluginPythonMcpProxyError as err:
            error_code = err.code
            response = jsonrpc_error_from_internal(body.id, err.code, err.message)
    logger.info(
        "[opencode_plugin_python_mcp] method=%s request_id=%s plugin_id=%s mcp_name=plugin_python latency_ms=%s error_code=%s",
        body.method,
        body.id,
        plugin_id,
