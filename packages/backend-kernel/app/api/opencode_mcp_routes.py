...
logger = get_logger("opencode_mcp_ui_routes")


def _tool_defs() -> list[Dict[str, Any]]:
    service = get_ui_tool_service()
    return [
        {
            "name": item.name,
...
async def opencode_mcp_ui(body: JsonRpcRequest):
    ...
    if body.method == "initialize":
        response = jsonrpc_response(
            body.id,
            {
                ...
                "capabilities": {"tools": {}},
                "serverInfo": {"name": "dawnchat-opencode-ui-mcp", "version": "0.1.0"},
            },
        )
    ...
    logger.info(
        "[opencode_mcp_ui] method=%s request_id=%s mcp_name=ui latency_ms=%s error_code=%s",
        body.method,
        body.id,
