...
- 为后续迭代提供一致的架构演进模型，避免在扩展页面、能力、状态时反复推翻。

相关总纲参考 [ai-gui-workspace-architecture.md](../../../internal/architecture/assistant/ai-gui-workspace-architecture.md)。
...
这些能力已在 [visual-voice-session-architecture.md](./visual-voice-session-architecture.md) 中明确，其意义在于：

- 会话级调度与插件业务执行已经成功解耦。
- Agent 已经可以一边改插件，一边调用插件进行视觉 + 语音交互。
- 当前链路已经从“单次页面渲染”进化到“多 step 会话执行”。

从代码现状看，当前 step 执行器仍处于 MVP 形态：
...
综合来看，当前架构已跑通闭环，但仍停留在“卡片讲解型会话”的早期阶段。
...
V1 规划以延续现有成果为前提，重点识别并处理会阻碍长期演进的结构性问题。
...
需要强调的是，V1 的重点不在命名空间本身，而在于借此完成职责收敛。
...
V1 不将 Scene、Capability、Flow 拆分为多个顶层协议入口。  
统一入口保持为：
...
该约束有助于保持宿主、Agent、运行时三方稳定性。

### 7.3 先统一 envelope，再细化内部语义

V1 先稳定以下 envelope 字段：
...
即使 V1 尚未完整实现恢复与 undo，也需为后续演进保留正确方向。

### 7.5 Agent 只做场景增量，不做 runtime 发明

平台侧负责提供稳定骨架，场景侧在该骨架上进行增量扩展：  

- 新资源
- 新 view
- 新 capability
- 新 guide 模式
- 不以重写 runtime 规则作为新增场景的默认路径。
...
该链路构成 V1 的稳定控制平面，不作为重构对象。

### 阶段 1：把当前执行器从“单 action”升级为“命名空间分发器”

当前执行器中，`actionHandlers` 仍是单层 map。

V1 目标不是简单增加 action 数量，而是让执行器具备如下分发能力：
...
据此，执行器可从“业务实现容器”升级为“runtime dispatcher”。
...
该顺序可确保 PDF 场景沿平台骨架扩展，而非由首个复杂场景倒逼架构临时发明。

---

## 12. V1 的成功标准

V1 达标不代表终局完成；达标意味着以下条件成立：
